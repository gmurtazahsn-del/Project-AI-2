...
async function recordUsage(env, data) {
  if (!env.SUPABASE_URL || !env.SUPABASE_SERVICE_ROLE_KEY) {
    console.error("Supabase usage tracking is not configured.");
    return;
  }

  try {
    await fetch(`${env.SUPABASE_URL}/rest/v1/usage_stats`, {
      method: "POST",
      headers: {
    ...
    console.error("Supabase usage tracking error:", error);
  }
}
...
var worker_default = {
  async fetch(request, env) {
    ...
    try {
      if ((url.pathname === "/" || url.pathname === "/chat" || url.pathname === "/chats") && request.method === "POST") {
      ...
      if (url.pathname === "/chats" && request.method === "PUT") {
...
async function chat(request, env) {
  ...
  try {
    const authUser = await authenticate(request, env.DB);
    usageUserId = authUser?.id || null;
  } catch (error) {
    // Guest users are allowed to chat.
  }
  ...
  if (
    ...
    if (creatorQuestion) {
      ...
      if (hasCodeWord) {
        return jsonResponse({
          reply: `I was created by ${CREATOR_NAME}. The project is part of a journey into coding and AI, with the goal of making human life easier, smoother, more comfortable, and faster.`
        });
      }

      return jsonResponse({
        reply:
          "I was created by a developer who is learning to code and exploring the world of AI, with the goal of building technology that can make everyday human life easier, faster, smoother, and more comfortable. Milky Way AI is part of that journey — a project built through learning, experimentation, curiosity, and a passion for creating something genuinely useful."
      });
    }
    ...
    if (
      ...
      return jsonResponse({
        reply:
          "You can tell me that you're my creator, but I can't verify someone's identity from a message alone. Creator-level access must be verified through the application's authorized authentication system."
      });
    }
  }
...
async function saveChats(request, env) {
  ...
  const chats = body.chats.slice(0, 100).map((chat2) => ({
    id: String(
      chat2.id || crypto.randomUUID()
    ),
    title: String(
      chat2.title || "New chat"
    ).slice(0, 100),
    updatedAt: Number(
    ...
    messages: Array.isArray(
      chat2.messages
    ) ? chat2.messages.slice(-100).map((message) => ({
...
async function createSession(db, userId) {
  ...
  const token = Array.from(bytes).map(
...
async function hashPassword(password) {
  ...
  return "pbkdf2$100000$" + bytesToHex(salt) + "$" + bytesToHex(
    new Uint8Array(bits)
  );
}
